...
定位：与 tests/ 分工——tests/ 守"机制"（guard / 沙箱 / 路由等确定性单测，30+ 个测试文件
已覆盖），本包守"模型行为"（prompt / 工具 docstring / tool.json 迭代时，LLM 涌现行为可能
静默回归：不建 plan、读任务调写工具、拒绝后死循环等）。

结构：
- tasks.py   任务集：Task（prompt / 预置文件 / 审批策略 / 检查项）+ 示例任务
- policy.py  审批策略：替代 TUI 里的人工 y/n（模拟人，按工具名批准 / 黑名单拒绝）
- runner.py  驱动器：复用 app/platform/turn.py 的 interrupt → Command(resume) park/resume 形态，
...
from .policy import allow_except, DEFAULT_DENY_TOOLS
from .runner import (
    EvalResult,
    build_eval_graph,
    run_suite,
