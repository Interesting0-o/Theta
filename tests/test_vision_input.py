...
前提（2026-09-14 实测）：**模型必须能看图**。当前配置的 `glm-4.7-flash` 会硬拒
（`400 / code 1210 / messages.content.type 参数非法，取值范围 ['text']`）；实测可用的是
`glm-4.6v-flash`（base64 与 http URL 都行，且**能同时挂工具**——agent 每轮都 `bind_tools`，
这是它能当主模型的前提）。免费层的 `glm-4v-flash` 只吃 URL、不吃 base64。
