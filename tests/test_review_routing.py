...
其余照常挂起，判定本体是 `app/agent/gates.py::free_shell_verdict`（纯函数，直接单测）。
"""
import asyncio
...
import app.agent.gates as gates
...
# ---------- run_command 的命令级免审判定（gates.free_shell_verdict）----------
...
def test_verdict_reads_the_policy_table_not_hardcoded_rules(monkeypatch):
    ...
    base = dict(gates.command_policy())

    widened = {
        **base,
        "free_commands": {**base["free_commands"], "git": base["free_commands"]["git"] | {"show"}},
    }
    monkeypatch.setattr(gates, "command_policy", lambda: widened)
    assert gates.free_shell_verdict({"command": "git show HEAD~1"}) is True  # 表里新加的子命令

    narrowed = {**base, "free_commands": {**base["free_commands"], "git": frozenset()}}
    monkeypatch.setattr(gates, "command_policy", lambda: narrowed)
    assert gates.free_shell_verdict({"command": "git status"}) is False  # 原本免审的，抽掉即收回
