"""app/agent/nodes.py::ToolNode.format_tool_result 的测试（对应 docs/EXCEPTION_DESIGN.md §10 L4）。
...
from app.agent.nodes import ToolNode
...
def test_format_tool_result_prefixes_error_type():
    ...
    assert ToolNode.format_tool_result(result) == "[workspace_violation] 路径 /etc 不在工作区/tmp 内"


def test_format_tool_result_no_error_type_returns_content():
    result = ToolResult(success=True, content="文件已写入: /ws/demo.txt")
    assert ToolNode.format_tool_result(result) == "文件已写入: /ws/demo.txt"
...
def test_format_tool_result_internal_error_prefix():
    ...
    assert ToolNode.format_tool_result(result).startswith("[internal_error]")


def test_format_tool_result_passthrough_string():
    assert ToolNode.format_tool_result("纯文本结果") == "纯文本结果"


def test_format_tool_result_dict_content():
    assert ToolNode.format_tool_result({"content": "来自字典"}) == "来自字典"
...
def test_format_tool_result_mcp_blocks_rebuilds_toolresult():
    ...
    assert ToolNode.format_tool_result([block]) == "1. 标题\n   http://x"
...
def test_format_tool_result_mcp_blocks_error_prefix():
    ...
    assert ToolNode.format_tool_result([block]) == "[invalid_argument] search_depth 必须是 basic"


def test_format_tool_result_mcp_blocks_plain_text():
    block = _mcp_block("纯文本结果")
    assert ToolNode.format_tool_result([block]) == "纯文本结果"


def test_format_tool_result_mcp_blocks_non_toolresult_json():
    block = _mcp_block('{"foo": 1}')
    assert ToolNode.format_tool_result([block]) == '{"foo": 1}'
...
def test_format_tool_result_mcp_blocks_drops_id_and_non_text():
    ...
    assert ToolNode.format_tool_result(blocks) == "第一段"
