...
模型是脚本化的（ScriptedChatModel）：按序吐 AIMessage，不花 token、不受提示词影响。
...
class ScriptedChatModel(BaseChatModel):
    ...
    最后一条**粘住**（不再 pop）：图在 tool_node 之后会回到 llm_node，收尾轮数不由测试决定，
    粘住尾条让"给完答复就 END"自然发生，测试不必猜轮数。
    `bind_tools` 返回自身——LLMNode 按工具集版本调它，缺这个会 NotImplementedError。
    """
...
async def _drive(workspace, session: str, command: str) -> dict:
...
def _run(monkeypatch, tmp_path, session: str, command: str) -> dict:
    """把模型换成脚本，跑一轮。工作区先 git init，让 `git status` 走成功路径。"""
    subprocess.run(
        ["git", "init", "-q", str(tmp_path)],
        capture_output=True, timeout=60, check=False,
    )
    script = ScriptedChatModel(replies=[_tool_call(command), AIMessage(content="做完了")])
    monkeypatch.setattr("app.agent.graph.get_main_chat_model", lambda: script)
    return asyncio.run(_drive(tmp_path, session, command))


def test_readonly_command_runs_without_asking(monkeypatch, tmp_path):
    """免审命令：图不挂起，命令**真的被执行**，回执作为 ToolMessage 回到对话。"""
    result = _run(monkeypatch, tmp_path, "e2e-free", "git status --porcelain --branch")
...
def test_write_command_parks_the_run_and_executes_nothing(monkeypatch, tmp_path):
    """需审批命令：图挂起、payload 带上命令原文，且**一次都没执行**（没有回执）。"""
    command = "git push https://example.com/o/r.git main"
    result = _run(monkeypatch, tmp_path, "e2e-review", command)
...
def test_free_and_review_only_differ_by_command_text(monkeypatch, tmp_path, command):
    """同一个工具、同一张图：分岔只由命令文本决定（决策点唯一性的回归钉子）。"""
    result = _run(monkeypatch, tmp_path, f"e2e-{abs(hash(command))}", command)
