...
- `/fast readme [语言]` `[已落地 2026-09-10]`：同为提示词型，但载荷**按参数渲染**（`prompts.py::fast_readme_prompt(args)`——命令表的 `prompt` 允许是"收 args 的函数"）：语言别名表认 `zh/cn/de/jp/fr…` 与"日语/德语"这类说法，**不给就默认英语**，不认识的记号原样带上（多数模型认语言代码）。同样是投提示词 → 走正常 turn：模型扫布局/依赖清单 → 在工作区根写或更新 `README.md`（撞审批）；纪律写明"只写能核实的事实、不许编造"。
