...
- `/fast readme [语言]` `[已落地 2026-09-10]`：同为提示词型，但载荷**按参数渲染**（`prompts.py::fast_readme_prompt(args)`——命令表的 `prompt` 允许是"收 args 的函数"）：语言别名表认 `zh/cn/de/jp/fr…` 与"日语/德语"这类说法，**不给就默认英语**，不认识的记号原样带上（多数模型认语言代码）。载荷按**开源项目 README 的通行结构**写：项目标识（徽章只放能核实的）/ 简介 / 功能特性 / **快速开始（最重要，60 秒能跑起来）** / 更多文档（用链接，别塞进 README）/ 贡献指南 / 许可证，外加可选章节（架构图、Changelog、API 速查、FAQ、致谢、赞助）；写作原则写明"先写给人看再写给机器看、信息分层、命令必须当前有效（过时的命令比没有更糟）、**不确定的宁可不写**"。另有「五、边界」一节（2026-09-10 实测后补）：**探索有预算**（5–8 次读取、别通读实现/测试/整份大文件）、**不要为了验证而执行命令**（命令从真实配置里摘；跑一遍确认会触发审批、可能很慢甚至挂住，尤其别跑训练/推理/下载）、**不要翻 `.git/` 内部**（要仓库来源用 `git remote -v`）、**允许先写粗版**（缺的留 `<!-- TODO -->`）。同样投提示词 → 走正常 turn：模型扫布局/依赖清单/已有 README → 在工作区根写或更新 `README.md`（撞审批），默认只写这一个文件。两条提示词共用的"落盘纪律 + 回执"抽在 `prompts.py::_landing_and_report()`。
