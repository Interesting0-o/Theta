...
**缺口（2026-09-16 复查发现）：只有单选，没有多选（复选框）。**

现状是「**一个选项** + 一段自由补充」，六个环节全按**标量**写死：工具签名、`Decision.option_index`、
闸门校验 `_normalize_ask_answer`、终端面板解析、回执文案、提示词。后果是模型的**绕路**：想表达
"这几项可以同时要"时，它只能*造一个组合选项*（"两者都要"）或指望用户把"1 和 3 都要"写进补充里
再由它自己解析——把工具面的缺失当提示词问题在解。这不是设计时权衡掉的（当时没考虑到），是落地
后复查才发现的。

要加的话动这六处（含两处需拍板）：
- **`Decision` 的形状**：`option_index: int | None` → 多值。**倾向**直接泛化成
  `option_indexes: tuple[int, ...]`（空 tuple = 没选），而不是并列加一个新字段——一个字段一种含义。
  同时**去掉 `option_text`**：工具本来就持有 `options`，消费端按序号取原文即可，不承重的冗余字段
  不进协议（同 2026-09-16 去掉 `additional_kwargs={"decision":"unanswered"}` 那次）。
- `ask_user` 加 `select: "one" | "many"`（默认 `one`），docstring 写清什么时候该多选；
- 闸门 `_normalize_ask_answer` 逐个序号判越界。**待拍**：越界的丢掉、还是整条作废？
- 终端面板解析 `1,3` 这类输入（`app/tui/ui.py::_ask` 现在是 `text.isdigit()` 单值）。**待拍**：
  多选下"一个都不选"是否等同单选的"没选中任何给定项"（单选时回车就是这个语义，多选应保持一致）；
- 回执文案要写明"**多选、共 N 项**"，否则模型会以为用户只答了一半；
- `SYSTEM_PROMPT` 的 `[提问 · ask_user]` 一节补"什么时候用多选"，否则模型会一直吃默认单选。
