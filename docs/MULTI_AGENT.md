...
> worker 失去了看仓库的能力，而它结构性拿不到 shell。
>
> 落点与后果：
> - **面内免审、面外回传**：worker 的 `ReviewNode` 读的是**同一份** `tool.json` 与
>   `command_policy.json`（两者都不依赖 toolset），所以**只读 git 子命令免审、完全不惊动人**；
>   写命令与联网才走本文的 HTTP 回传通道。等于把"逐次 interrupt"这条轴扩到了 worker 侧。
> - **知情质量的补偿**：worker 的审批 payload 多了一个 `subtask`（**派给它的子任务原文**），
>   面板渲染成 `子任务: …` 一行。主 agent 的对话人一直在跟，worker 的推理人看不到——只给
>   command + description 时，人是在"审一条来源不明的命令"。
> - **代价（如实记）**：worker 的"只读"从**结构性排除**降级为"免审面 + 人审"。面内的只读性仍是
>   结构性的（白名单即结构），面外则靠审批面板那一眼。
> - **进程表隔离**：worker 的终端是它**自己的 server 实例**（每子任务一个 session_id），所以它起的
>   进程不在主 agent 的 `process_list` 里、主 agent 也杀不掉，随子任务结束回收。
> - **同时修掉的链路缺陷（当日基座审计）**：worker 侧 httpx 超时曾写死 60s，**小于**主侧长轮询的
>   120s block——人在 60–120 秒之间才按的决定会无人领取、整条子任务失败。现两侧共用
>   `app/schema/approval_schema.py::INBOX_BLOCK_SECONDS`。

审批有两个正交轴，须分别定：

1. **谁审批（已钉）**：审批单点留在主侧。worker 不自行放行写/命令——**审核权始终在主侧**（2026-09-15 下放终端后仍如此：worker 能发起，但放行由人）。
