...
五、边界（这条命令是"**快速**写文档"，不是做代码审计——别把预算烧在收集上）
- **探索有预算**：通常 **5–8 次读取就够**——看布局、依赖/构建清单、入口文件、已有的 README
  即可。**不要通读实现、不要读测试文件、不要把大文件整份读完**（确需细节就 search_content
  定位到几行）。README 写的是"**怎么用起来**"，不是"怎么实现"。
- **不要为了验证而执行命令**：命令与示例要**从仓库的真实配置/脚本里摘**（pyproject /
  package.json / Makefile / 已有文档 / CI 配置）；**拿不准就宁可不写**——**不要跑一遍去确认它
  能不能跑**：那会触发人工审批、可能很慢甚至挂住。**尤其不要跑训练、模型推理、数据下载这类重
  命令。**
- **不要翻 `.git/` 内部**（`.git/config`、`.git/HEAD` 之类）：要仓库来源就 `git remote -v` 一条，
  拿不到就算了（徽章/链接宁可不写，也别去挖）。
- **允许先写粗版**：信息够就先落一版，缺的部分在文件里留 `<!-- TODO: 待补 … -->` 交给用户，
  而不是继续挖——"快速给出一版可用的"比"攒够了再写"更符合这条命令的定位。

怎么做：
1. 先扫现状再写：get_directory_tree / list_dir 看布局，读依赖与构建清单、入口文件，确认测试怎么跑
   （看配置/文档即可，**别读测试文件**），以及**已有的 README.md**（有就先 read_file 看懂，
   在它基础上更新，而不是推倒重来）。
