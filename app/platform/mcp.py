"""MCP **宿主侧**运行时：连接配置 + 工具加载 + **运行体常驻**（docs/SKILL_DESIGN.md §12）。

**位置**（2026-09-21 从 `app/agent/mcp.py` 搬来）：本模块与同目录的 `tool_results.py` 是
**主进程这一侧的 MCP 边界**——一个管"运行体怎么活"、一个管"返回值怎么解"。按
docs/ARCHITECTURE.md §4 的 A/B/C，它三条都不答（不是"agent 知道什么"、不在图里、也不是
"agent 能做什么"）；独立的旁证是**消费者在基座**：`loop.py` 管它的关闭、`evaluation/runner.py`
与 `mcp_service/sub_agent.py` 各取一个入口，`graph.py` 只用它的 `load_mcp_tool` 两次。

**依赖方向**：本模块**不 import app.agent**（故 `import app.platform` 不触发 .env，仍成立）；
反向是允许的——`app.agent.*`（graph / nodes / tools）可以 import 本模块，这是单向的。基座里
唯一 import `app.agent` 的地方仍是 `runtime.py` 的懒加载建图。
...
1. **连接配置**：`_build_servers` 按工作区构造 file_io / terminal / dispatch / web_search 的
   stdio 连接（dispatch = 派发 worker 的入口，随它搬来；见 `_build_servers` 的转发链说明）；
...
（`app/platform/tool_results.py::format_tool_result`）因此不受影响。
...
def _inbox_env() -> dict[str, str]:
    """主侧审批收件箱地址 → 需要它的 server 的 extra_env（没起收件箱 → 空表）。

    空值按"未设置"处理（与本仓 `.env` 惯例一致）：传个空变量只会被下游当成噪声，不如不传。
    """
    url = os.environ.get("AGENT_INBOX_URL", "").strip()
    return {"AGENT_INBOX_URL": url} if url else {}


def _build_servers(workspace_path: str) -> dict[str, Connection]:
    """按当前配置构造要拉起的 MCP server 连接集合（不实际拉起子进程，便于单测）。

    - file_io / terminal / dispatch：必选，cwd 设为工作区根；
    - web_search：TAVILY_API_KEY 为空则跳过并告警（可选能力不阻塞主流程）。

    **dispatch 要额外带 `AGENT_INBOX_URL`**：它派生 worker，而 worker 的审批要回主侧收件箱，
    这条转发链是 `主进程 env →（本处 extra_env）→ dispatch server env →（它内部的
    _worker_child_env）→ worker`。**漏掉本处这一跳不会报错，只会静默发往默认端口**——主侧一换
    端口（AGENT_INBOX_PORT）审批就无人应答（2026-09-15 审计发现过同一形态的失效）。主侧启动
    收件箱后会把**实际**地址写进本进程 env（见 app/platform/approvals.py）；没起收件箱的宿主
    （evaluation / langgraph dev）拿不到该键，worker 会回退到默认端口——与改造前的行为一致。
    """
    servers: dict[str, Connection] = {
        ...
        "dispatch": stdio_connection(
            "mcp_service.dispatch", workspace_path, _inbox_env()
        ),
    }
